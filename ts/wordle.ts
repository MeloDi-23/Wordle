...
function trySubmit(row: number) {
    if(!inputElements[row].every(e=>e.value.match(/^[A-Z]$/))) {
        return;
    }
...
function monitorKey(event: KeyboardEvent) {
    let match;
    let tar = event.target as HTMLInputElement;
    let row = parseInt(tar.id[4]);
    let col = parseInt(tar.id[5]);
    let flag = false;
    console.log(event);
    if(match = event.code.match(/^Key([A-Z])$/)) {
        tar.value = match[1];
        tar.style.animation = 'emphAnimation 400ms ease-in-out';
        tar.onanimationend = (e) => (e.target as HTMLInputElement).style.animation = 'none';
        let count = -1;
...
function gameOver(win: boolean) {
    ...
    _prompt.id = 'prompt';
    if(win) {
        console.log(`- Wordle: use ${currentRow} times to win`);
        _prompt.innerHTML = 'CONGRATULATIONS!';
        if(currentRow <= 2)
            _prompt.innerHTML += '<br><span class="hint">YOU\'RE GENIUS!</span>';

    } else {
        _prompt.innerHTML = `YOU\'VE FAILED!<br><span class="hint">ans: ${targetString}</span>`;
    }
    containerDiv.onanimationend = () => {document.body.appendChild(_prompt); containerDiv.onanimationend = null;};
    _prompt.onanimationend = () => {
        _prompt.style.animation = 'none';
